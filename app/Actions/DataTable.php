...
class DataTable extends Action
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $totalRecordswithFilter = $this->model->when($searchValue != null, function ($q) use ($searchValue) {
            $q->where('tasks.text', 'like', '%' . $searchValue . '%');
            $q->orWhere('tasks.status', 'like', '%' . $searchValue . '%');
            $q->orWhereIN('tasks.employee_id', User::where('first_name', 'like', '%' . $searchValue . '%')
                ->orWhere('last_name', 'like', '%' . $searchValue . '%')->pluck('id')->toArray());
